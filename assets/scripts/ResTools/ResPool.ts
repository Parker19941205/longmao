...
export class ResPool {
    ...
    public loadRes() {
        ...
        resArgs.onCompleted = (error: Error, resource: any) => {
            let urls = resLoader.getUrlByAsset(resource);
            if (!error && urls) {
                this.addNewResUrl(urls);
            } else {
                this.addNewResUrl(resArgs.url)
            }
            if (SaveCompleted) {
                SaveCompleted(error, resource);
            }
        };
        if (resArgs.url && resArgs.onCompleted && resArgs.type) {
            resLoader.loadRes(resArgs.url, resArgs.type, resArgs.onCompleted)
        }
